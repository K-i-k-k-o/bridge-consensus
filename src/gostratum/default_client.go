...
var walletRegex = regexp.MustCompile("consensus:[a-z0-9]+")
...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "consensus:") {
		return CleanWallet("consensus:" + in)
	}
	...
	return "", errors.New("unable to coerce wallet to valid consensus address")
}
